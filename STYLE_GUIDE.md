...
2. **Date**: Automatically synced from filename date prefix
3. **Last Modified**: Automatically synced from filename date prefix
...
**Examples:**
- `2024-03-15-how-to-build-scalable-apis.md`
- `2023-12-01-understanding-microservices.md`
- `2025-01-10-ai-trends-for-developers.md`

**Rules:**
1. **Date prefix**: YYYY-MM-DD format - this is the single source of truth for post dates
2. **Slug format**: Lowercase, hyphens only, no special characters
3. **Descriptive**: File name should clearly indicate the post topic
4. **Automatic sync**: Run `./sync_dates.sh` to sync front matter dates with filename dates

**Note**: The filename date prefix automatically sets both `date` and `lastmod` fields in the front matter. To change a post's date, rename the file and run the sync script.
...
- `declarative-programming` - Declarative systems and paradigms
- `proof-systems` - Logical proof systems and engines
- `hybrid-systems` - Systems combining multiple approaches
...
6. **Em dash spacing**: Always use spaces around em dashes for readability: "Consider quantum immortality — the speculative idea that" not "Consider quantum immortality—the speculative idea that"
7. **Australian English spelling**: Use Australian English conventions:
   - `-ise` not `-ize` (organise, realise, customise, prioritise)
...
### Heading Hierarchy (CRITICAL for Accessibility)

**Always follow proper heading sequence** - H1 → H2 → H3 → H4 (never skip levels):

```markdown
# Post Title (H1) - Automatically generated from front matter title
## Main Section (H2)
### Subsection (H3) 
#### Sub-subsection (H4)
## Another Main Section (H2) - Back to H2, not H3
```

**Common Mistakes to Avoid:**
- ❌ `H1 → H3` (skipping H2)
- ❌ `H2 → H4` (skipping H3)  
- ❌ Using H3 for sections that should be H2

**Template Requirements:**
- Post title: Always H1 (handled automatically)
- Main content sections: H2
- Subsections within content: H3
- Template sections (Related Posts, etc.): H2 (not H3)
